...
function QualityBadge({ score }: { score: number }) {
  ...
  const color = pct >= 80 ? "text-success" : pct >= 60 ? "text-primary" : "text-destructive";
  return <span className={`font-mono text-sm font-bold ${color}`}>{pct}%</span>;
}

function TrackCard({ track, index, selected, onSelect }: { track: PlayerTrack; index: number; selected: boolean; onSelect: () => void }) {
  return (
    <motion.button
      ...
      className={`w-full text-left p-4 rounded-xl transition-all ${
        selected ? "bg-primary/10 border-2 border-primary glow-primary" : "bg-secondary/40 border border-border hover:border-primary/30"
      }`}
    >
      <div className="flex items-start gap-3">
        <div className={`w-12 h-12 rounded-lg flex items-center justify-center ${selected ? "bg-primary text-primary-foreground" : "bg-muted"}`}>
    ...
    <div className="min-h-screen flex flex-col lg:flex-row court-pattern">
      <div className="flex-1 flex items-center justify-center p-6">
        <div className="relative w-full max-w-2xl aspect-video rounded-2xl overflow-hidden bg-secondary/30 border border-border">
          {videoClip?.url ? (
            <video src={videoClip.url} controls className="w-full h-full object-contain" />
          ) : (
            ...
            return (
              <motion.div
                ...
                className={`absolute border-2 rounded cursor-pointer transition-colors ${
                  isSelected ? "border-primary bg-primary/10" : isHovered ? "border-accent bg-accent/10" : "border-muted-foreground/30 bg-transparent"
                }`}
                style={{
                  left: `${box.x * 100}%`,
                  top: `${box.y * 100}%`,
              ...
              >
                <span className={`absolute -top-5 left-0 text-xs font-mono px-1.5 py-0.5 rounded ${
          ...
          {avatar && (
            <div className="absolute bottom-3 left-3 flex items-center gap-2 glass rounded-lg px-3 py-2">
              <img src={avatar.thumbnailUrl} alt="Avatar" className="w-8 h-8 rounded-full object-cover border border-primary/30" />
      ...
      <div className="w-full lg:w-96 border-t lg:border-t-0 lg:border-l border-border bg-card/50 p-6 overflow-y-auto">
        <div className="mb-6">
          ...
          <p className="text-sm text-muted-foreground mt-1">{tracks.length} players detected · Choose who to replace</p>
        </div>

        <div className="space-y-3">
          {sortedTracks.map((track, i) => (
            <TrackCard key={track.trackId} track={track} index={i} selected={selectedTrackId === track.trackId} onSelect={() => setSelectedTrackId(track.trackId)} />
          ))}
        </div>

        <div className="mt-6 space-y-3">
          <Button onClick={() => setCurrentStep("replacing")} disabled={!selectedTrackId} className="w-full glow-primary" size="lg">
            Start Replacement <ArrowRight className="w-4 h-4 ml-2" />
          </Button>
          <Button variant="ghost" onClick={() => setCurrentStep("upload")} className="w-full text-muted-foreground">← Back</Button>
        </div>
      </div>
    </div>
  );
